...
async def create_manual_booking(b: Booking, admin=Depends(get_current_admin)):
    try:
        ...
        return {"ok": True, "booking": b.model_dump()}
...
async def update_booking(
    ...
    if 'total_price' in patch:
        try:
            patch['total_price'] = float(patch['total_price'])
            # LOG DI DEBUG - Vedremo nei log se il prezzo passa da qui
            logging.info(f"DEBUG: Aggiornamento prezzo per {decoded_id} a {patch['total_price']}")
        except:
            pass
    ...
    updated = await db.bookings.find_one({'id': decoded_id}, {'_id': 0})
    return updated
...
async def cancel_and_refund(booking_id: str, payload: RefundRequest, admin=Depends(get_current_admin)):
    ...
    if refund_amount > 0:
        tx = await db.payment_transactions.find_one({'booking_id': decoded_id, 'payment_status': 'paid'})
        pi = (tx or {}).get('payment_intent_id') or b.get('payment_intent_id')
        if pi:
            try:
                await asyncio.to_thread(stripe.Refund.create, payment_intent=pi, amount=int(round(refund_amount * 100)))
            except: logging.exception('Refund failed')
    await db.bookings.update_one({'id': decoded_id}, {'$set': {'status': 'cancelled', 'refund_amount': refund_amount}})
    return {'ok': True}
...
async def balance_reminder(booking_id: str, admin=Depends(get_current_admin)):
    ...
    ok = await send_email_async(b['guest_email'], "Saldo", email_balance_reminder_html(b, settings))
    await db.bookings.update_one({'id': decoded_id}, {'$set': {'last_reminder_at': datetime.now(timezone.utc).isoformat()}})
    return {'ok': ok}
